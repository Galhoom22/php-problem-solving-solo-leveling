# Gate 146 — Merge Intervals

Status: Not cleared
Related skill: Pattern-16-intervals
Difficulty: Medium
Gate Rank: B
Min Hunter Rank: B-Rank
XP Reward: 90
Source Platform: LeetCode

## System Brief
Merge overlapping intervals.

See LeetCode for examples.

## What This Gate Is Testing
Sort intervals then merge overlaps.

## Learn First (System Tutorial)
- Sort by start
- Extend end when overlapping

## Backend Link (Why this matters at work)
### Direct
- Booking, schedule merge, deployment windows.
### Indirect
- Range thinking in business rules.
### Job takeaway
- "Intervals are calendar logic."

## Clear Rule
Mark ✅ in `02-Problems/README.md` and update the Status Window only after you can explain and clear it cleanly.

## Source
Source: LeetCode — https://leetcode.com/problems/merge-intervals/
